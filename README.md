...
CSSwitch 是一个给 Claude Science 使用的本地配置转换器。它把 Science 的推理请求转换并接入你自己的模型 API，可配置 DeepSeek、通义千问、Kimi、MiniMax、GLM、OpenRouter、中转站或自定义兼容端点。当前源码还包含默认关闭、尚未随正式版发布的 Codex OAuth 实验接入。
...
- 一个可用的第三方模型 API Key；仅实验 Codex 路径改用 CSSwitch 独立 OAuth
...
当前源码中的 Codex 能力尚未随 `v0.6.0` 发布，默认关闭。测试源码时，应在「高级设置」中明确启用，使用 CSSwitch 独立登录，再新建 Codex 配置；无需填写 API Key、`base_url` 或固定模型。Science 启动后从 “More models” 选择 `Codex / …` 动态模型。它不复用或修改原生 Codex 登录，真实账号与 live provider 验收仍未完成。
...
| Codex（源码实验，默认关闭） | CSSwitch 独立 OAuth + Responses/SSE | 动态读取当前账号模型；不复用原生 Codex 登录，真实账号验收未完成 |
...
- Codex 实验凭据只保存在 CSSwitch 自有 macOS Keychain 项；配置只保存固定 opaque reference。CSSwitch 不读取、复制、覆盖或删除原生 `~/.codex` 登录。
...
- Codex OAuth 接入目前仅存在于实验源码、默认关闭，尚未完成用户真实 OAuth、live provider、签名/公证或发布附件验收。
