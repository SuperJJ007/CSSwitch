...
状态：**browser-only 登录、OAuth 后自动 profile、动态 GPT-5.6 目录兼容、双 App 数据根隔离与无签名前置的私有文件认证已进入源码和自动 Gate**。Codex 功能不要求 Apple Developer 身份或正式签名。2026-07-17 的 no-signing Acceptance 已由用户完成浏览器 OAuth、动态模型目录、Science 选择 `Codex / GPT-5.6-Sol` 和文本推理验收；最终 0.7.0 发布附件仍需按独立 artifact 重新建立 hash、打包和分发证据。历史与当前候选边界见 [Acceptance 安装证据](../evidence/investigations/2026-07-17-codex-browser-only-acceptance.md)。
