...
当前自动 Gate 已覆盖 mock OAuth、配置迁移、协议转换、动态目录、旧 UI 与生命周期。RM-35～RM-42 中需要真实账号的项目仍必须等用户在隔离 Acceptance app 中亲自授权后完成，因此不能据源码测试声称真实账号、可用额度、两个以上 live 模型或发布包已验证。

## 参考实现边界

实现语义按以下优先级取证：

1. `openai/codex@2e4f55608b4ad26d9c48ea45a6fcd20bfd5e9fe8`（Apache-2.0）：`codex-rs/login/src/{server.rs,pkce.rs,auth/manager.rs,auth/revoke.rs}` 的 OAuth、刷新、账号语义，以及同一固定 SHA 的模型目录 endpoint / 字段语义；
...
```text
csswitch-gateway codex-auth login
csswitch-gateway codex-auth status
csswitch-gateway codex-auth logout
```

三个命令的 stdout 必须且只能是一行 UTF-8 JSON，schema 为：

```json
{
  "schema_version": 1,
  "ok": true,
  "command": "login | status | logout",
  "status": {
    "authenticated": true,
...
失败时 `ok=false`，用 `error={code,message,retryable}` 替代 `status`。稳定退出码为：0 成功；2 参数 / schema；3 未登录；4 浏览器或用户终止；5 callback 超时；6 平台、Keychain、锁或本地状态不可用；7 OAuth 网络 / 协议；8 内部错误。稳定 `error.code` 至少覆盖 `invalid_arguments`、`not_authenticated`、`unsupported_platform`、`auth_busy`、`browser_open_failed`、`callback_timeout`、`oauth_denied`、`oauth_protocol_error`、`keychain_unavailable`、`auth_state_invalid`、`auth_changed`。输出不得包含邮箱、authorization code、token、PKCE verifier、state、nonce、完整上游 body 或 Keychain payload。

`login` 使用系统浏览器、Authorization Code + PKCE-S256 和至少 256-bit 随机 `state`。回调只绑定 `127.0.0.1`，按顺序尝试端口 1455、1457；均不可安全绑定时直接失败。登录最多等待五分钟。浏览器打开失败立即关闭 listener，不能交换或保存凭据；带匹配 state 的 OAuth error 是终态 `oauth_denied`；错误或重放 state 返回 400 并继续等待，不得交换 code；首个有效 callback 成为唯一终态并立即关闭 listener，之后的双 callback 不得覆盖结果。
...
Keychain 不可用、拒绝或返回歧义项时 fail closed，不降级到文件。production build 只允许系统浏览器、固定官方 endpoint 和 macOS Keychain；fake browser、fake OAuth、fake Keychain 只能通过 Rust 测试注入 trait 使用，不能通过生产环境变量注入 endpoint、token 或 Keychain payload。`logout` 先 best-effort revoke，再删除 CSSwitch 自有 Keychain 项；不得删除原生 Codex 凭据。revoke 失败不阻止本地删除，删除失败则返回错误并保持 gateway fail closed。
...
Codex 是现有 provider 架构中的一种 capability 组合，不新增第二套入口系统。旧 UI 只增加：实验开关、登录/退出、脱敏状态、动态模型列表和错误提示；新 UI 后续复用相同 backend contract。登录态不是 UI 专属守卫：正式 proxy、scratch / 模型发现和 Science 一键启动都在后端边界先调用受管 `codex-auth status`，只有 CSSwitch 自有状态明确为 authenticated 才允许继续；未登录或 sidecar 状态不可确认时，在启动任何 Codex gateway 或改变隔离 Science 状态前 fail closed。

login/logout 前 Tauri 必须先停止受管 Codex serve，不能停止其他 provider 的 Science 或 gateway。若身份无法确认或 stop 失败，login/logout fail closed，不继续 auth mutation、不杀未知 PID。关闭实验开关只停止自有 Codex 链路，不删除凭据；停止失败时 UI 据实保留运行 / 错误状态。用户显式 logout 才删除 CSSwitch Codex 凭据。
